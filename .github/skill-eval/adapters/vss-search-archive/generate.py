...
INGESTION_PREAMBLE = (
    ...
    "Treat the two fixtures as one staged batch: perform both upload-URL handshakes and both file transfers "
    "before calling either `/complete`; preserve both upload responses and UUIDs, then boundedly require one "
    "VST `/sensor/list` response that simultaneously contains the exact `warehouse_sample` and canonical "
    "`warehouse-ladder` name/UUID pairs. Only after capturing that live registration evidence, call and "
    "start both separate `/complete` requests before waiting for either, then validate both responses independently. "
    "If an Agent-returned public upload authority differs from an already-recorded origin whose literal non-global "
    "IP proves it is the host fallback, preserve the exact validated `/vst/api/v1/storage/file` path and use that "
    "recorded origin authority without probing the "
    "rejected public candidate again; otherwise use the returned URL unchanged. Do not replace the simultaneous pre-completion listing "
    "with post-completion polling, two observations from different times, logs, or index documents. "
...
OPERATION_PREAMBLE = (
    ...
    "into the reply. For nonempty results, use one block per hit and clearly summarize the CLI evidence fields; a particular heading "
    "or prose layout is not required. Use a verification question only under the all-unverified rule above. "
    "Preserve the CLI evidence fields, print every hit's complete exact `screenshot_url` on a visible `Media URL:` line, "
    "and explicitly say that similarity scores are retrieval evidence rather than visual confirmation. "
    "A URL checked only inside a command was not reported. Before replying, require the number of visible `Media URL:` "
    "lines to equal the number of successfully validated hits; do not omit URLs to make a compact table. Preserve the exact returned "
...
VERIFICATION_PREAMBLE = (
    ...
    "that clip against the complete supplied visual intent and return only the structured result contract. "
    "Pass only `VIDEO_URL` and `QUESTION` to that skill; do not pass sensor or stream IDs, timestamps, "
    "model or endpoint details, or request/retry instructions. "
    "Invoke the skill exactly once. Finish all read-only endpoint selection before inference; prefer a "
    "configured `VLM_REMOTE_URL`/`VLM_REMOTE_MODEL` whose authenticated `/models` probe succeeds, then freeze "
    "endpoint, model, clip, and upload format. After the first chat-completions POST, do not change any of those "
    "choices or post again for an HTTP, auth, media, model, or semantic failure. Only a 2xx but malformed JSON "
    "answer permits one same-endpoint, same-model, same-clip repair POST. "
    ...
    "evidence. Keep the final response implementation-neutral: report only the verdict, boolean criteria, "
    "and visual evidence, without model/vendor names, VLM terminology, endpoints or URLs, sensor/stream "
    "IDs or UUIDs, timeline mechanics, request counts, retry details, or an operational summary. The final "
    "reply must not contain the term `VLM`, including in a technical-failure explanation."
)
