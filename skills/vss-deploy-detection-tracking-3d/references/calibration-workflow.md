...
5. Preserve the final ordered RTSP URL list. The final standalone deployment will register streams with `scripts/add-streams.sh`, which waits on REST `/api/v1/ready` before registration.
