...
- For RTSP: REST `/api/v1/ready` reports `ds-ready=YES` or `scripts/add-streams.sh` completes its built-in readiness wait, registered stream count equals `NUM_CAMS`, registered IDs exactly match generated camInfo IDs with no duplicates/extras, every expected source has recent non-zero FPS, and both `mdx-raw` and `mdx-bev` offsets grow while streams are active.
